...
if (count($argv) !== 2) {
    exit("Usage: php post_build_commands.php [composer file]\n");
}
...
if ($json['scripts'] == $newScripts) {
    exit("composer.json is already configured for Laravel\n");
}

$json['scripts'] = $newScripts;
file_put_contents($composerFile, json_encode($json, JSON_PRETTY_PRINT));

echo "composer.json is now configured for Laravel\n";
